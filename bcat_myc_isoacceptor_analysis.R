setwd("/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/bcat_myc/counts_analysis/collapsed_anticodon/")
...
sample_information <- read_tsv("samplefile_bcat.txt")

seq_data <- read_tsv("anticodon_counts_DESeq2input.txt")

seq_data <- seq_data %>%
  select(-c(WT3, WT5, bcat_myc1))

seq_data <- seq_data[apply(seq_data[, -1], MARGIN = 1, function(x) all(x > 10)), ]

newcolnames <- c("Gene", "Null1", "Null2", "Null3", "bcat_myc1", "bcat_myc2", "bcat_myc3", "bcat_myc4")
...
conditionCol <- match(sample_information$Condition, c("Null", "bcat_myc"))
...
conditionCol <- match(sample_information$Condition, c("Null", "bcat_myc")) + 1

# above we have bcat_mycated a colour vector 
...
# MDM2 <- results(ddsObj_results, contrast = c("Condition", "bcat_myc", "Null"), alpha = 0.05)

library(apeglm)

bcat_myc <- lfcShrink(ddsObj_results, coef = "Condition_bcat_myc_vs_Null", type = "apeglm")

#################################################################################################################################
# PCA post batch correction 

ddsObj$Mouse <- factor(rep(c("1", "2", "3", "4", "5", "6", "7")))
...
bcat_myc_table <- as.data.frame(bcat_myc) %>%
  rownames_to_column("Gene") %>%
  rename(log2FC = log2FoldChange, FDR = padj)

# save the DESeq2 results table for future reference 

write_tsv(bcat_myc_table, "/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/bcat_myc/counts_analysis/collapsed_anticodon/rawcounts_collapsedanticodon_fulland3prime_DESeq2results.txt")

# reformatt the table so it can be used for tRNA barplot generation 

# step 1 ... remove all rows that are non-tRNAs

barplot_input <- slice(bcat_myc_table, 1:49) # only take the tRNA data 
...
for (iso in c("Ala", "Arg", "Asn", "Asp", "Cys", "Gln", "Glu", "Gly", "His", "Ile", "Leu", "Lys", "Met", "Phe", "Pro", "SeC", "Ser", "Thr", "Trp", "Tyr", "Val", "iMet")) {
  ...
  temp_plot <- ggplot(data = df2, aes(x = Label2, y = log2FC, fill = wobble, label = ifelse(FDR < 0.05, "*", "ns")))+
    ...
    ggtitle(iso, "Null vs bcat/myc in Mouse Liver")
...
png("MA_plot_Nullvsbcat_myc.png")

plotMA(bcat_myc, alpha = 0.05,
       main = "Null vs bcat/myc in Mouse Liver")
...
df <- as.data.frame(bcat_myc_table)
...
png("Nullbcat_myc_volcanoplot.png")
...
ggplot(data = new_df, aes(x = log2FC, y = -log10(FDR), colour = DE))+
  ...
  ylab("-log10(FDR)")+
  xlab("log2FC")+
  ggtitle("Null vs bcat/myc in Mouse Liver")+
  theme_bw()+
...
de <- as.integer(bcat_myc$padj <= 0.05 & !is.na(bcat_myc$padj))

normCounts <- log2(counts(ddsObj))

glXYPlot(
  x = bcat_myc$log2FoldChange,
  y = -log10(bcat_myc$pvalue),
  xlab = "log2FC",
  ylab = "-log10(FDR)",
  side.xlab = "Condition",
  side.ylab = "Log(CPM)",
  ...
  status = de,
  #anno = shrink_Day4_ConVsbcat_myc[, c("GeneID", "Symbol", "Description")],
  folder = "volcano")
...
sigGenes <- as.data.frame(bcat_myc_table) %>% 
  top_n(200, wt=-FDR) %>% 
  pull("Gene")
...
ggplot(data = at_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("A/T at the anticodon wobble position - Null vs bcat/myc in Mouse Liver")
...
ggplot(data = gc_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("G/C at the anticodon wobble position - Null vs bcat/myc in Mouse Liver")
...
normalized_counts <- mutate(normalized_counts, Null_Mean = rowMeans(cbind(normalized_counts$Null1, normalized_counts$Null2, normalized_counts$Null3)))

normalized_counts <- mutate(normalized_counts, bcat_myc_Mean = rowMeans(cbind(normalized_counts$bcat_myc1, normalized_counts$bcat_myc2, normalized_counts$bcat_myc3, normalized_counts$bcat_myc4)))
...
df3_melt <- tidyr::gather(df3, variable, value, Null1:bcat_myc4)

df3_melt <- mutate(df3_melt, Condition = case_when(variable == "Null1" ~ "Null",
                                                   variable == "Null2" ~ "Null",
                                                   variable == "Null3" ~ "Null",
                                                   variable == "bcat_myc1" ~ "bcat_myc",
                                                   variable == "bcat_myc2" ~ "bcat_myc",
                                                   variable == "bcat_myc3" ~ "bcat_myc",
                                                   variable == "bcat_myc4" ~ "bcat_myc"))
...
for (iso in c("Ala", "Arg", "Asn", "Asp", "Cys", "Gln", "Glu", "Gly", "His", "Ile", "Leu", "Lys", "Met", "Phe", "Pro", "SeC", "Ser", "Thr", "Trp", "Tyr", "Val", "iMet")) {
  ...
  normalized_counts_graphs <- ggplot(data = df4, aes(x = Label2, y = value, colour = Condition, group = Condition))+
    ...
    ggtitle(iso, "Null vs bcat/myc - Normalized Counts")
  ...
  tRNA_barplot <- ggplot(data = df2, aes(x = Label2, y = log2FC, fill = wobble, label = ifelse(FDR < 0.05, "*", "ns")))+
    ...
    theme_bw()+
    theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
    ggtitle(iso, "Null vs bcat/myc - tRNA LFCs")
...
df3_melt_mean <- tidyr::gather(df3_mean, variable, value, Null_Mean:bcat_myc_Mean)

df3_melt_mean <- mutate(df3_melt_mean, Condition = case_when(variable == "Null_Mean" ~ "Null - Mean of the normalized counts",
                                                             variable == "bcat_myc_Mean" ~ "bcat_myc - Mean of the normalized counts"))
...
for (iso in c("Ala", "Arg", "Asn", "Asp", "Cys", "Gln", "Glu", "Gly", "His", "Ile", "Leu", "Lys", "Met", "Phe", "Pro", "SeC", "Ser", "Thr", "Trp", "Tyr", "Val", "iMet")) {
  ...
  normalized_counts_graphs_mean <- ggplot(data = df4_mean, aes(x = Label2, y = value, colour = Condition, group = Condition))+
    ...
    theme_bw()+
    theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
    ggtitle(iso, "Null vs bcat/myc - Mean Normalized Counts")
...
df_gc_wobble_melt <- tidyr::gather(gc_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_gc <- ggplot(data = df_gc_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("G/C at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
GC_at_wobble <- ggplot(data = gc_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("G/C at the anticodon wobble position", "Null vs bcat/myc - tRNA LFCs")
...
df_at_wobble_melt <- tidyr::gather(at_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_at <- ggplot(data = df_at_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("A/T at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
AT_at_wobble <- ggplot(data = at_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("A/T at the anticodon wobble position", "Null vs bcat/myc")
...
df_gc_wobble_melt_mean <- tidyr::gather(gc_ending_figure_mean, variable, value, Null_Mean:bcat_myc_Mean) 
...
df_at_wobble_melt_mean <- tidyr::gather(at_ending_figure_mean, variable, value, Null_Mean:bcat_myc_Mean) 
...
AT_at_wobble_mean <- ggplot(data = at_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("A/T at the anticodon wobble position", "Null vs bcat/myc - tRNA LFCs")
...
df_a_wobble_melt <- tidyr::gather(a_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_a <- ggplot(data = df_a_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("A at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
A_at_wobble <- ggplot(data = a_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("A at the anticodon wobble position", "Null vs bcat/myc")
...
df_t_wobble_melt <- tidyr::gather(t_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_t <- ggplot(data = df_t_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("T at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
T_at_wobble <- ggplot(data = t_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("T at the anticodon wobble position", "Null vs bcat/myc")
...
df_g_wobble_melt <- tidyr::gather(g_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_g <- ggplot(data = df_g_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("G at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
G_at_wobble <- ggplot(data = g_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("G at the anticodon wobble position", "Null vs bcat/myc")
...
df_c_wobble_melt <- tidyr::gather(c_ending_figure, variable, value, Null1:bcat_myc4) 
...
normalized_counts_wobble_graphs_c <- ggplot(data = df_c_wobble_melt, aes(x = Label2, y = value, colour = Condition, group = Condition))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(size = 12))+
  ggtitle("C at the anticodon wobble position", "Null vs bcat/myc - Normalized Counts")
...
C_at_wobble <- ggplot(data = c_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("C at the anticodon wobble position", "Null vs bcat/myc")
