setwd("/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/nomultimap_FedvsStarved/counts_analysis/tRNAsonly/apeglm_minCount10/collapsed_anticodons/")
...
sample_information <- read_tsv("/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/nomultimap_FedvsStarved/counts_analysis/tRNAsonly/apeglm_minCount10/collapsed_anticodons/sampleinformation.txt")

seq_data <- read_tsv("/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/nomultimap_FedvsStarved/counts_analysis/tRNAsonly/apeglm_minCount10/collapsed_anticodons/anticodon_counts_DESeq2input.txt")
...
plotPCA(vsd, "batch") # this should give you the variance associated with the effect of the batch 
...
plotPCA(vsd, intgroup = c("batch")) # plotting the residuals 
...
write_tsv(FedvsStarved_table, "/mnt/data/BMOHAMED/TRAX_nov2019/tRAX-master/nomultimap_FedvsStarved/counts_analysis/tRNAsonly/apeglm_minCount10/collapsed_anticodons/rawcounts_collapsedanticodon_fulland3prime_DESeq2results.txt")

# reformatt the table so it can be used for tRNA barplot generation 

# step 1 ... remove all rows that are non-tRNAs

barplot_input <- slice(FedvsStarved_table, 1:49) # only take the tRNA data 
...
for (iso in c("Ala", "Arg", "Asn", "Asp", "Cys", "Gln", "Glu", "Gly", "His", "Ile", "Leu", "Lys", "Met", "Phe", "Pro", "SeC", "Ser", "Thr", "Trp", "Tyr", "Val", "iMet")) {
  ...
  temp_plot <- ggplot(data = df2, aes(x = Label2, y = log2FC, fill = wobble, label = ifelse(FDR < 0.05, "*", "ns")))+
    ...
    #geom_text(vjust = 0, nudge_y = ifelse(df2$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(df2$FDR < 0.05, 6, 4))+
    xlab("Isoacceptor")+
...
ggplot(data = new_df, aes(x = log2FC, y = -log10(FDR), colour = DE))+
  ...
  ylab("-log10(FDR)")+
  xlab("log2FC")+
  ggtitle("Fed vs Starved in BJ-5TA fibroblasts")+
  theme_bw()+
...
glXYPlot(
  ...
  side.xlab = "Condition",
  side.ylab = "Log(CPM)",
  main = "Fed vs Starved in BJ-5TA fibroblasts",
  counts = normCounts,
  groups = group,
...
ggplot(data = at_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(at_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(at_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-4, 4))+
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("A/T at the anticodon wobble position - Fed vs Starved in BJ-5TA fibroblasts")
...
ggplot(data = gc_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(gc_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(gc_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-4, 4))+
  theme_bw()+
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1), plot.margin = margin(0.5,0.5,0.5,2, "cm"), axis.title = element_text(face = "bold", size = 12))+
  ggtitle("G/C at the anticodon wobble position - Fed vs Starved in BJ-5TA fibroblasts")
...
A_at_wobble <- ggplot(data = a_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(a_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(a_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-3, 3))+
  theme_bw()+
...
T_at_wobble <- ggplot(data = t_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(t_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(t_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-3, 3))+
  theme_bw()+
...
G_at_wobble <- ggplot(data = g_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(g_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(g_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-4, 4))+
  theme_bw()+
...
C_at_wobble <- ggplot(data = c_ending, aes(x = Label2, y = log2FC, fill = Isodecoder, label = ifelse(FDR < 0.05, "*", "ns")))+
  ...
  geom_text(aes(label = ifelse(FDR < 0.05, "*", paste("p=", round(FDR, 4), sep = "")), 
                ...
                fontface = "bold"))+
  scale_y_continuous(expand = expansion(mult = 0.1))+
  #geom_text(vjust = 0, nudge_y = ifelse(c_ending$log2FC < 0, -0.1, 0.1), fontface = "bold", size = ifelse(c_ending$FDR < 0.05, 6, 4))+
  xlab("Isoacceptor")+
  ylab(expression(paste(Log[2],"FC",sep="")))+
  #ylim(c(-4, 4))+
  theme_bw()+
